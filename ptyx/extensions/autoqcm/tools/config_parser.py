...
def real2apparent(original_q_num: int, original_a_num: int, config: dict, doc_id: str) -> Tuple[int, int]:
...
def is_answer_correct(q_num: int, a_num: int, config: dict, doc_id: str, use_original_num: bool = True) -> bool:
    if use_original_num:
...
def get_correct_answers(config: dict, use_original_num: bool = True) -> Dict[str, Dict[int, Set[int]]]:
    ...
    for doc_id in config['ordering']:
        ...
        for i, q in enumerate(questions, start=1):
            ...
            if use_original_num:
                correct_answers[q] = {a for (a, _is_correct) in answers[q]
                                              if _is_correct}
            else:
                correct_answers[i] = {j for (j, (a, _is_correct)) in enumerate(answers[q], start=1)
                                              if _is_correct}
