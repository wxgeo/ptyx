...
def _test_singularity_and_append(code: str, answers_list: list, question: str) -> str:
    code = code.strip()
...
class AutoQCMLatexGenerator(LatexGenerator):
    ...
    def _parse_NEW_QUESTION_tag(self, node):
        ...
        self._autoqcm_pick_and_parse_children(node, children=node.children,
                                              target='VERSION',
                                              )
    ...
    def _parse_NEW_ANSWER_tag(self, node):
        ...
        if not self.context.get('ALLOW_SAME_ANSWER_TWICE'):
            ...
            functions.append(partial(_test_singularity_and_append, l=self.autoqcm_answers,
    ...
    def _parse_ANSWERS_LIST_tag(self, node):
        ...
        for ans in answers:
            _test_singularity_and_append(ans, answers_certified_unique, self.current_question[0])
