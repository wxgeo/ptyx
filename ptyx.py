...
if __name__ == '__main__':
    ...
    for input_name in arguments:
        ...
        if latex_generator.preparser.extensions_loaded:
            # Save pTyX code generated by extensions (this is used for debuging,
            # but extensions can also save some data this way using #COMMENT tag).
            ptyx_file_name = os.path.join(os.path.dirname(input_name),
                                '.%s.plain-ptyx' % os.path.basename(input_name))
            with open(ptyx_file_name, 'w') as plain_ptyx_file:
                plain_ptyx_file.write(latex_generator.preparser.plain_ptyx_code)
